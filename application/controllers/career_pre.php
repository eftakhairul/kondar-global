            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($blocked_email)) {
                    ...
                    if ($datelimit > $blocked_email[0]->dte_block) {   //block time expired
                        ...
                        if ($int_TR < 0)
                            $int_TR = 0;
                        $str = "The email " . $blocked_email[0]->str_email . "  is blocked in the section " . $blocked_email[0]->region . " . Therefore, please use an alternative email or wait " . $int_TR . "  minutes to use this email again within our website. Thank you";
...
add date: ' . date('d-m-Y H:i', $blockTime1);
                ...
                if (!empty($check_user2)) {
                    $this->session->set_flashdata('success', lang('You have already applied for this job. Please try another job.'));
                    redirect('career/apply_form/' . $id);
                }
                $attempt = 1;
...
//                $this->email->from('hr@kondarglobal.ca', 'KGT HR Department');
                ...
                $this->email->send();
                $this->session->set_flashdata('success', sprintf(lang("Thank you for your interest to work with KGT. Your job application is well received and a copy is already sent to your email : %s. We should contact you in case your job application is shortlisted."), $user_data['email']));
