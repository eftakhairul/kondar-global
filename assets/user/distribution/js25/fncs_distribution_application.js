...
function runCountDownClck(str_targetdiv, int_seconds)
{
    // alert(int_seconds);
...
function formReset(form_id){
    ...
    document.getElementById('applicant').disabled = true;
    ...
    //document.getElementById('companysize_medium').disabled = true;
    // document.getElementById('companysize_big').disabled = true;
    document.getElementById('companystart_brake_pads').disabled = true;
    document.getElementById('companystart_filters').disabled = true;
    document.getElementById('companystart_brake_lining').disabled = true;
    document.getElementById('txt_id_salesbrief').disabled = true;
    document.getElementById('chk_id_agree').disabled = true;
    document.getElementById('chk_id_signup').disabled = true;

    document.getElementById('sel_id_indoor_sales').disabled = true;
    document.getElementById('sel_id_outdoor_sales').disabled = true;
...
function fn_Handle_Fields(str_type, gen_value, str_id_next_field){
    switch(str_type){
        case 'txt':
            if(gen_value != ''){
                ...
                if(str_id_next_field == 'country'){
                    ...
                    $('#' + str_id_next_field).set('disabled', false);
                    $('.' + str_id_next_field).set('disabled', false);
                }   
            }
            break;
        ...
        case 'license':
            if(gen_value == 1)
                document.getElementById(str_id_next_field).disabled = false;
            break;
...
function fn_Handle_Block(int_case, bln_edit){
    ...
    if($('#frm_id_distribution_application').hasClass('resetForm')){
        ...
        success: function (data) {
            ...
            if(data['end']!='end'){
                ...
                if(typeof clock !== "undefined")
                    clock.reset();
                // alert('aaaaaaaaaaa')
                // alert(data['str_verification_error']);
                $('#code_verification').modal('hide');
                $('#block_box').modal('show');
...
function fn_Handle_Submit_Verification(int_check_signup){
    ...
    if(int_check_signup == 0){
        fn_Delete_Cookies('*');
        //  fn_Send_Verification_Code('send_mail');
